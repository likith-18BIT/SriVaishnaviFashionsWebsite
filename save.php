...
if(isset($_GET['delete'])){
    $id = $_GET['delete'];
    $sql = "select * from savelist where id = ".$id;
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_assoc($result);
        // $image = $row['image'];
        // unlink($upload_dir.$image);
        $sql = "delete from savelist where id=".$id;
...
if(!empty($_SESSION)){ 
    $user_id = $_SESSION['id'];
    $sql = "SELECT s.id,s.user_id,s.product_id,m.product_name,m.cost,m.description,m.image,m.image4,m.image2,m.image3,m.image5 FROM savelist s
    LEFT JOIN model_products m
                    					...
                    					while($row = mysqli_fetch_assoc($result)){?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="save.php?delete=<?php echo $row['id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure to delete this record?')"><i class="fa fa-trash-alt"></i></a>
                                                </td>
                                            </tr>
